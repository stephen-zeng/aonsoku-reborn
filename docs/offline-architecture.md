...
客户端不再提供音质档位选择。所有音频请求(在线播放、预加载、离线缓存、导出下载)统一使用 Subsonic `/stream` 端点且不传递 `maxBitRate` 参数，由服务器自行决定返回的音频质量。

### 不做的事

- 不提供客户端音质档位选择
- 不按网络自动切换质量(§8 会在受限网络下暂停下载)
- 不做"按设备自适应"
...
| 同步线程 | Web Worker(Comlink) | 主线程 + 分块让出(`requestIdleCallback`) | 迁移到 Web Worker,通过 Comlink 桥接主线程状态更新(Zustand/queryClient/cacheIndexStore)。Worker 内独立实例化 Dexie 连接同一 IDB,认证配置通过 `initAuth`/`updateAuth` 注入。主线程 `SyncWorkerAdapter` 设立 Comlink 回调将进度/失效信号写回 Zustand + queryClient。Worker 不可用时自动降级到主线程 `MetadataSyncService`。 |
