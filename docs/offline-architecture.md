...
| 同步线程 | 主线程 + 分块让出(`requestIdleCallback`) | Web Worker 完整迁移 | 主线程实现足够:Dexie IDB 本身异步,主要卡顿点是大响应反序列化 + bulkPut。用 2000 行/块的 `bulkPutInChunks` + `yieldToMain` 可把 T3 分摊到多个事件循环,避免一次性阻塞。完整 Worker 迁移需要把 zustand 状态更新 + queryClient 失效通过 postMessage 桥接,实现复杂度 vs 当前收益失衡,先延后。若实测 10 万+ 库下仍有可感卡顿,再升级。 |
