...
function Container({ children, className, ...rest }: ActionsContainerProps) {
  return (
    <div
      {...rest}
      className={cn("w-full my-3 md:my-6 flex items-center gap-1", className)}
    >
      {children}
    </div>
  );
...
function Button({
  ...
  const button = (
    <ComponentButton
      className={cn(
        "rounded-full w-12 h-12 md:w-14 md:h-14 ease-linear duration-100 transition",
...
function EllipsisIcon() {
  return (
    <EllipsisVertical className="w-5 h-5 drop-shadow-md" strokeWidth={2} />
  );
}
